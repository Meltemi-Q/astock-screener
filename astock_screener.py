...
def _default_report_year():
    """按当前日期推导最近可用年报年份：默认上一年；每年 4 月底前年报未全，再回退一年。"""
    now = datetime.now()
    y = now.year - 1
    if now.month <= 4:   # 早春(1~4月)年报披露未完成，回退到再上一年更稳
        y -= 1
    return y
...
# ============================================================
CONFIG = {
    "report_year": None,        # 主报告期（用最近一期年报）；None=按当前年份推导，脚本会自动回退到上一年若数据不足
...
# report_year 为 None 时按当前年份推导（避免跨年后锁死在过期年报）
if CONFIG["report_year"] is None:
    CONFIG["report_year"] = _default_report_year()
...
SPOT_WARNINGS = []  # 行情抓取过程中的告警（限流/空页等），会写入产物 meta，避免静默丢数据
...
def _atomic_write(path, data, mode="w", encoding="utf-8", newline=None):
    """原子写：先写同目录 tmp 文件再 os.replace，避免 open('w') 就地截断在崩溃/并发时留下半截文件。"""
...
def get_json(url, ttl_hours=None):
    ...
    d = json.loads(raw)
    try:
        _atomic_write(fp, json.dumps(d, ensure_ascii=False), mode="w", encoding="utf-8")
    except Exception:
        pass
    return d
...
def fetch_spot():
    ...
    for fs, bname in SPOT_BOARDS:
        ...
        while True:
            ...
            expect_more = pn * pz < total
            # 空页但理论上还有数据 → 大概率限流/异常空页：带退避多次重试，避免静默漏数据。
            # 首页(pn==1)拿到空页(常伴 total=0)同样可疑，不能当"该板块真的没股票"静默略过。
            if not diff and (expect_more or pn == 1):
                for attempt in range(CONFIG["retries"]):
                    time.sleep(0.5 * (attempt + 1))
                    data = (get_json(url + f"&_r={attempt+1}", ttl_hours=0).get("data")) or {}
                    diff = data.get("diff") or []
                    if total in (None, 0):
                        total = data.get("total") or 0
                    if diff:
                        break
                if not diff:
                    msg = (f"行情·{bname} 第{pn}页疑似被限流/异常空页(total={total}, "
                           f"已取{len(out)}只)，可能丢失后段数据")
                    print(f"  ⚠ {msg}")
                    SPOT_WARNINGS.append(msg)
                    break
            for r in diff:
                out[str(r.get("f12"))] = r
    ...
    def _fetch_board(fs, bname):
        ...
        while True:
            ...
            if total is None:
                total = data.get("total") or 0
            diff = data.get("diff") or []
            expect_more = pn * pz < total  # 按 total 判断本页之后是否还应有数据
            # 空页但理论上还有数据 → 大概率是限流/异常空页，而非真的末页：带退避多次重试。
            # 首页(pn==1)拿到空页(常伴 total=0)同样可疑，不能当"该板块真的没股票"静默略过。
            if not diff and (expect_more or pn == 1):
                for attempt in range(CONFIG["retries"]):
                    time.sleep(0.5 * (attempt + 1))
                    ...
                    if diff:
                        break
                # 重试后仍空：不静默丢弃后段，记录 warning 进产物并中止本板块翻页
                if not diff:
                    msg = (f"行情·{bname} 第{pn}页疑似被限流/异常空页(total={total}, "
                           f"已取{len(out)}只)，可能丢失后段数据，建议 --quotes-fresh 重跑")
                    print(f"  ⚠ {msg}")
                    SPOT_WARNINGS.append(msg)
                    break
            for r in diff:
                out[str(r.get("f12"))] = r
            # 只有"取满 total"才是正常翻到末页；diff 为空但 expect_more 已在上方处理
            if pn * pz >= total or not diff:
                break
            pn += 1
            time.sleep(CONFIG["page_sleep"])
...
def build_records(year):
    ...
    for code, sp in spot.items():
        ...
        total_shares = fnum(sp.get("f38"))  # 总股本(股)

        nm = (netp / rev * 100.0) if (netp is not None and rev not in (None, 0)) else None
        # 现金流/利润统一到合并口径：总经营现金流(=每股OCF×总股本) ÷ 归母净利，避免"每股OCF÷EPS"的股本基准错配
        # (MGJYXJJE 每股OCF按总股本，BASIC_EPS 归母EPS按加权股本，直接相除口径不一致)
        # 注意：分子含少数股东现金流、分母为归母净利，对少数股东占比高的公司该比值偏乐观，L0 排雷时留意
        ocf_to_profit = None
        if ocf_ps is not None and total_shares not in (None, 0) and netp not in (None, 0) and netp > 0:
            ocf_to_profit = (ocf_ps * total_shares) / netp
        elif ocf_ps is not None and eps not in (None, 0) and eps > 0:
            ocf_to_profit = ocf_ps / eps  # 兜底：无总股本时退回每股口径近似
        ...
        # SUMSHEQUITY_RATIO 是小数口径(0.38=38%)，统一乘 100 转百分数，与阈值 max_goodwill_ratio(30.0) 同口径
        gwr = (fnum((gw_i.get(code) or {}).get("SUMSHEQUITY_RATIO")) or 0.0) * 100.0  # 不在商誉表=无商誉=0
...
def write_csv(records, path):
    ...
    _atomic_write(path, buf.getvalue(), mode="w", encoding="utf-8-sig", newline="")
...
if(META.warnings&&META.warnings.length){
 var wtxt=META.warnings.map(function(w){return esc(w)}).join("；");
 document.getElementById("sub").innerHTML+='<div style="color:var(--red);margin-top:4px">⚠ 行情抓取告警：'+wtxt+'</div>';
}
...
function renderLB(){
 ...
 rows.forEach(function(r,i){
  ...
  html1+='<div class="lb-row"><span class="lb-rank '+rc+'">'+(i+1)+'</span>'
    +'<span class="lb-code">'+esc(r.code)+'</span><span class="lb-name">'+esc(r.name)+'</span>'
    +'<span class="lb-tag '+tg+'">'+esc(r.tier)+'</span><span class="lb-score">'+esc(r.sc)+'</span></div>';
 });
...
function rowHTML(r){
 var cells=COLS.map(function(c){var k=c[0],v=r[k];
  ...
  if(k==="code"){
   var ec=encodeURIComponent(r.code),sc=esc(v);
   var dv = r.deep
    ? '<a href="deep_dives/report.html?code='+ec+'" class="code" title="查看深度研报">'+sc+'</a>'
    : '<a href="deep_dives/report.html?code='+ec+'" class="code pending" title="生成深度研报">'+sc+'</a>';
   return '<td class="l">'+dv+'</td>';
  }
  if(k==="name"){
   var nm = (r.warn?"⚠":"")+esc(v),ec2=encodeURIComponent(r.code);
   var dv2 = r.deep
    ? '<a href="deep_dives/report.html?code='+ec2+'" style="color:inherit;text-decoration:none" title="查看深度研报">'+nm+'</a>'
    : '<a href="deep_dives/report.html?code='+ec2+'" style="color:#64748b;text-decoration:none" title="生成深度研报">'+nm+'</a>';
...
def write_html(records, path, year, total_eval, tierN):
    ...
    meta = {"year": year, "total": total_eval, "ts": time.strftime("%Y-%m-%d %H:%M"),
            ...
            "warnings": list(SPOT_WARNINGS)}  # 行情限流/空页告警，前端展示，避免静默丢数据
    html = HTML_TEMPLATE \
        .replace("__DATA__", json.dumps(data, ensure_ascii=False)) \
        .replace("__INDS__", json.dumps(inds, ensure_ascii=False)) \
        .replace("__META__", json.dumps(meta, ensure_ascii=False))
    _atomic_write(path, html, mode="w", encoding="utf-8")
...
<script>location.replace({target_js})</script>
    ...
    for name in ("index.html", STABLE_SCREEN_NAME):
        _atomic_write(os.path.join(OUT_DIR, name), html, mode="w", encoding="utf-8")
...
def write_md(tierA, tierB, tierC, path, year, total_eval):
    ...
    _atomic_write(path, "\n".join(parts), mode="w", encoding="utf-8")
